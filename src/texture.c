...
{
  // TODO Bounds check
  const size_t tx = (size_t)roundf((float)texture->width * x);
  const size_t ty = (size_t)roundf((float)texture->height * y);

  const size_t offset = ty * texture->width + tx;

  return ((color_t*)texture->data)[offset];
}
